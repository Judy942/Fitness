...
class _FinishWorkoutScreenState extends State<CompleteWorkoutScreen> {
  ...
  Future<void> _fetchExerciseData() async {
    Map<String, dynamic> exercises = await _userService.fetchDataMap(
      'http://192.168.102.186:8055/api/workouts/${widget.workoutSchedule["workout_id"]["id"]}',
    );
    setState(() {
                    ...
                    return CompletedExerciseRow(
                      ...
                      onDelete: (exerciseId) {
                        _userService.deleteData(
                          'http://192.168.102.186:8055/items/workout_schedule_exercise/$exerciseId',
                        );
                      },
                    );
                  }).toList(),
                          ...
                          if (value == true) {
                            if (!widget.completedExercise.any((exercise) =>
                              ...
                              userService.postData(
                                'http://192.168.102.186:8055/items/workout_schedule_exercise',
                                {
                                  "workout_schedule_id":
              ...
              Stack(
                ...
                children: [
                  ...
                  ClipRRect(
                    ...
                    child: Image.network(
                      'http://192.168.102.186:8055/assets/${widget.wObj["exercise_id"]["image"]}',
                      width: 90,
                      height: 90,
                      fit: BoxFit.fill,
