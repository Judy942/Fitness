...
import 'package:shared_preferences/shared_preferences.dart';
  ...
  log('${prefs.getString('userToken')}'); // Log token
  return prefs.getString('userToken');
}
class UserService {
  static const String baseUrl = 'http://192.168.133.100:8055/users/me';
  ...
  Future<bool> updateData(String path, Map<String, dynamic> data) async {
    ...
    try {
      final response = await http.patch(Uri.parse(path),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json'
          },
  ...
  Future<int> getCompletedExerciseId(
      int workoutScheduleId, int exerciseId) async {
    String? token = await getToken();
    print("Token: $token"); // Debugging purpose

    final response = await http.get(
      Uri.parse(
          'http://192.168.133.100:8055/items/workout_schedule_exercise?workout_schedule_id=$workoutScheduleId&exercise_id=$exerciseId'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json'
      },
